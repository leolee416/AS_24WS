...
class FramesPublisherNode{
 ...
 public:
  ...
  void onPublish(const ros::TimerEvent&){
    ...
    // *** FILL IN *** 
    // AV1
    AV1World.setOrigin( tf::Vector3(cos(time), sin(time), 0.0) ); // origin tf of AV1
    tf::Quaternion q1;
    q1.setRPY(0, 0, time);
    AV1World.setRotation(q1); // Rotation of AV1

    // AV2
    AV2World.setOrigin( tf::Vector3(sin(time), 0.0, cos(2*time)) ); // origin tf of AV1
    tf::Quaternion q2(0,0,0,1);
    AV2World.setRotation(q2); // Rotation of AV1
    // 3. Publish the transforms, namely:
    //     - AV1World with frame_id "worlds", child_frame_id "av1"
